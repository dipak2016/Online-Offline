...
-(void)Insertbulkdata:(NSArray *)arr
{
    ...
    for (int i=0; i<arr.count; i++)
    {
        if (sqlite3_open([get_dbpath UTF8String], &dbsql)==SQLITE_OK)
        {
            ...
            NSString *query=[NSString stringWithFormat:@"insert into state(st_nm,st_cod)values('%@','%@')",[[arr objectAtIndex:i]objectForKey:@"stnm"],[[arr objectAtIndex:i]objectForKey:@"stcode"]];
